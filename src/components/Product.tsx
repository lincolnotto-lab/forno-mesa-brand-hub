...
import { fadeUp, staggerContainer, clipRevealHorizontal, viewportConfig, ease } from "@/lib/motion";

const gridImages = [
  { src: "/product-display.jpg", alt: "Exposição em balcão de padaria" },
  { src: "/product-packaging.jpg", alt: "Embalagem Forno & Mesa" },
  { src: "/product-hand.jpg", alt: "Consumidor com o produto" },
];
    ...
    <section id="produto" className="bg-cream min-h-[110svh] py-20 md:py-32">
      <div className="max-w-[1440px] mx-auto px-6 md:px-10 lg:px-14">
        <motion.div
          ...
          className="flex flex-col items-start lg:items-center gap-6 mb-16 md:mb-24"
        >
          ...
          <motion.h2 variants={fadeUp} className="text-h2-section font-serif text-graphite lg:text-center">
            ...
            <br />
            e forte na <em className="italic text-accent-red">memória</em>.
          </motion.h2>
          ...
          {gridImages.map((img) => (
            ...
            >
              <img
                ...
                height={480}
              />
              <div className="absolute bottom-0 left-0 right-0 h-px bg-accent-red scale-x-0 group-hover:scale-x-100 transition-transform duration-700 origin-left" style={{ transitionTimingFunction: "cubic-bezier(0.22, 1, 0.36, 1)" }} />
            </motion.div>
          ))}
        </div>
      </div>
